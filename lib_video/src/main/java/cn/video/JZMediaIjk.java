...
public class JZMediaIjk extends JZMediaInterface implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnVideoSizeChangedListener, IMediaPlayer.OnCompletionListener, IMediaPlayer.OnErrorListener, IMediaPlayer.OnInfoListener, IMediaPlayer.OnBufferingUpdateListener, IMediaPlayer.OnSeekCompleteListener, IMediaPlayer.OnTimedTextListener, CacheListener {
    ...
    public void prepare() {
        ...
        mMediaHandler.post(() -> {
            ...
            try {
                HttpProxyCacheServer proxy = VideoCacheSingleton.getSingleton().getProxy(jzvd.jzvdContext);
                proxy.registerCacheListener(this, jzvd.jzDataSource.getCurrentUrl().toString());
                String proxyUrl = proxy.getProxyUrl(jzvd.jzDataSource.getCurrentUrl().toString());
                Log.i("HttpProxyCacheServer", "jzDataSource url ="+ jzvd.jzDataSource.getCurrentUrl().toString()
                        +", proxyUrl = "+ proxyUrl);
//                if (proxy.isCached(jzvd.jzDataSource.getCurrentUrl().toString())) {
//                    Log.i("HttpProxyCacheServer", "已缓存");
//                    ijkMediaPlayer.setDataSource(proxyUrl);
//                } else {
//                    Log.i("HttpProxyCacheServer", "未缓存");
...
//                }
    ...
    public void onCacheAvailable(File cacheFile, String url, int percentsAvailable) {
        Log.i("HttpProxyCacheServer", "onCacheAvailable cacheFile ="+ cacheFile + ", url ="+url +" , percentsAvailable ="+ percentsAvailable);
    }
}
