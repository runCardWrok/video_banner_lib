...
import com.youth.banner.util.MapScrollSpeedManger;
...
public class MapBanner<T, BA extends BannerAdapter<T, ? extends RecyclerView.ViewHolder>> extends FrameLayout implements BannerLifecycleObserver {
    ...
    public MapBanner(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MapBanner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        MapScrollSpeedManger.reflectLayoutManager(this);
        addView(mViewPager2);
        ...
        public void onPageScrollStateChanged(int state) {
            ...
            if (state == ViewPager2.SCROLL_STATE_DRAGGING || state == ViewPager2.SCROLL_STATE_SETTLING) {
                ...
                if (mTempPosition != INVALID_VALUE && mIsInfiniteLoop) {
                    if (mTempPosition == 0) {
                        setCurrentItem(getRealCount(), true);//?
    ...
    static class AutoLoopTask implements Runnable {
        private final WeakReference<MapBanner> reference;

        AutoLoopTask(MapBanner banner) {
            this.reference = new WeakReference<>(banner);
        }

        @Override
        public void run() {
            MapBanner banner = reference.get();
     ...
     * @return
     */
    public MapBanner setIntercept(boolean intercept) {
     ...
     * @return
     */
    public MapBanner setCurrentItem(int position) {
     ...
     * @return
     */
    public MapBanner setCurrentItem(int position, boolean smoothScroll) {
        getViewPager2().setCurrentItem(position, smoothScroll);
        return this;
    }

    public MapBanner setIndicatorPageChange() {
        if (getIndicator() != null) {
    ...
    public MapBanner removeIndicator() {
        if (getIndicator() != null) {
     ...
     */
    public MapBanner setStartPosition(int mStartPosition) {
     ...
     */
    public MapBanner setUserInputEnabled(boolean enabled) {
     ...
     */
    public MapBanner addPageTransformer(@Nullable ViewPager2.PageTransformer transformer) {
     ...
     */
    public MapBanner setPageTransformer(@Nullable ViewPager2.PageTransformer transformer) {
        getViewPager2().setPageTransformer(transformer);
        return this;
    }

    public MapBanner removeTransformer(ViewPager2.PageTransformer transformer) {
     ...
     * 添加 ItemDecoration
     */
    public MapBanner addItemDecoration(RecyclerView.ItemDecoration decor) {
        getViewPager2().addItemDecoration(decor);
        return this;
    }

    public MapBanner addItemDecoration(RecyclerView.ItemDecoration decor, int index) {
     ...
     */
    public MapBanner isAutoLoop(boolean isAutoLoop) {
     ...
     */
    public MapBanner setLoopTime(long loopTime) {
     ...
     * 设置轮播滑动过程的时间
     */
    public MapBanner setScrollTime(int scrollTime) {
     ...
     * 设置banner的适配器
     */
    public MapBanner setAdapter(BA adapter) {
        if (adapter == null) {
     ...
     * @return
     */
    public MapBanner setAdapter(BA adapter, boolean isInfiniteLoop) {
     ...
     */
    public MapBanner setDatas(List<T> datas) {
        if (getAdapter() != null) {
     ...
     */
    public MapBanner setOrientation(@Orientation int orientation) {
     ...
     * 改变最小滑动距离
     */
    public MapBanner setTouchSlop(int mTouchSlop) {
     ...
     * 设置点击事件
     */
    public MapBanner setOnBannerListener(OnBannerListener<T> listener) {
        if (getAdapter() != null) {
     ...
     * </p>
     */
    public MapBanner addOnPageChangeListener(OnPageChangeListener pageListener) {
     ...
     * @param radius 圆角半径
     */
    public MapBanner setBannerRound(float radius) {
    ...
    public MapBanner setBannerRound2(float radius) {
     ...
     */
    public MapBanner setBannerGalleryEffect(int itemWidth, int pageMargin) {
     ...
     * @param pageMargin     页面间距,单位dp
     */
    public MapBanner setBannerGalleryEffect(int leftItemWidth, int rightItemWidth, int pageMargin) {
     ...
     */
    public MapBanner setBannerGalleryEffect(int itemWidth, int pageMargin, float scale) {
     ...
     * @param scale          缩放[0-1],1代表不缩放
     */
    public MapBanner setBannerGalleryEffect(int leftItemWidth, int rightItemWidth, int pageMargin, float scale) {
        if (pageMargin > 0) {
     ...
     * @param itemWidth item左右展示的宽度,单位dp
     */
    public MapBanner setBannerGalleryMZ(int itemWidth) {
     ...
     * @param scale     缩放[0-1],1代表不缩放
     */
    public MapBanner setBannerGalleryMZ(int itemWidth, float scale) {
        if (scale < 1 && scale > 0) {
     ...
     * 设置轮播指示器(显示在banner上)
     */
    public MapBanner setIndicator(Indicator indicator) {
     ...
     */
    public MapBanner setIndicator(Indicator indicator, boolean attachToBanner) {
        removeIndicator();
    ...
    public MapBanner setIndicatorSelectedColor(@ColorInt int color) {
    ...
    public MapBanner setIndicatorSelectedColorRes(@ColorRes int color) {
        setIndicatorSelectedColor(ContextCompat.getColor(getContext(), color));
        return this;
    }

    public MapBanner setIndicatorNormalColor(@ColorInt int color) {
        if (getIndicatorConfig() != null) {
    ...
    public MapBanner setIndicatorNormalColorRes(@ColorRes int color) {
        setIndicatorNormalColor(ContextCompat.getColor(getContext(), color));
        return this;
    }

    public MapBanner setIndicatorGravity(@IndicatorConfig.Direction int gravity) {
    ...
    public MapBanner setIndicatorSpace(int indicatorSpace) {
        if (getIndicatorConfig() != null) {
    ...
    public MapBanner setIndicatorMargins(IndicatorConfig.Margins margins) {
        if (getIndicatorConfig() != null && getIndicatorConfig().isAttachToBanner()) {
    ...
    public MapBanner setIndicatorWidth(int normalWidth, int selectedWidth) {
        if (getIndicatorConfig() != null) {
    ...
    public MapBanner setIndicatorNormalWidth(int normalWidth) {
        if (getIndicatorConfig() != null) {
            getIndicatorConfig().setNormalWidth(normalWidth);
        }
        return this;
    ...
    public MapBanner setIndicatorSelectedWidth(int selectedWidth) {
        if (getIndicatorConfig() != null) {
            getIndicatorConfig().setSelectedWidth(selectedWidth);
        }
        return this;
    ...
    public MapBanner setIndicatorRadius(int indicatorRadius) {
        if (getIndicatorConfig() != null) {
    ...
    public MapBanner setIndicatorHeight(int indicatorHeight) {
        if (getIndicatorConfig() != null) {
    ...
    public MapBanner addBannerLifecycleObserver(LifecycleOwner owner) {
        if (owner != null) {
